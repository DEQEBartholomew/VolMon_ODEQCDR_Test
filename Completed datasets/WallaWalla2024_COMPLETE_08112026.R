...
##change work directory to your dataset##
...
df5.results <- df4.results %>%
  ...
  #odeqcdr::dql_update(rows = c(), "B", "No post audit, downgraded to B data") %>%
  #odeqcdr::dql_update(rows = c(), "E", "No pre or post audit") %>%
  #odeqcdr::dql_update(rows = c(), "C", "Anomalous temperature jump") %>%
  #odeqcdr::dql_update(rows = c(), "E", "Audit over 30 minutes") %>%
  #odeqcdr::dql_update(rows = c(), "B", "B audit, B data") %>%
  #odeqcdr::dql_update(rows = c(), "C", "Logger out of water or in very shallow water") %>%
